...
| `MODEL_NAME` | `mistralai/Voxtral-Mini-3B-TTS-2603` | `app/model.py` | Hugging Face model id passed to `Omni(model=...)`. |
| `HF_TOKEN` / `HUGGING_FACE_HUB_TOKEN` | _(unset)_ | Hugging Face Hub (tokenizer + weights) | Read token for **gated** Mistral models. Must be non-empty if set; an empty value causes **401 Invalid username or password** from the Hub. Accept the model license on huggingface.co, then set one of these variables (they are mirrored if only one is set). |
